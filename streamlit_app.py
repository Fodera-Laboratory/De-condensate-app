...
with tab_pls:
    ...
    with _pc2:
        ...
        _random_seed = st.number_input(
            "Random seed", min_value=0, max_value=2**31 - 1, value=42, step=1,
            help="Seed for the train/test split and repeated k-fold CV. "
                 "Change this to verify that results are robust across different splits.",
        )
                        ...
                        with st.spinner("Training triple protein1+protein2+crowder PLS model…"):
                            try:
                                pls_protein = an.build_triple_pls_model(
                                    ...
                                    test_size=_test_size, random_state=int(_random_seed),
                                )
    ...
    if "results" not in st.session_state or not st.session_state["results"]:
        ...
        if not _can_reconstruct:
            ...
            with _col_water:
                ...
                st.plotly_chart(_fig_profiles, use_container_width=True)
                if _c_water is None:
                    st.info("No solvent reference selected. Choose files in the 'Solvent reference spectra' expander above.")
...
with tab_cls:
    ...
    if _cls_ref_src:
        try:
            _df_cls  = _read_csv_src(_cls_ref_src)
            ...
            _cls_comp_ids = _sel_cls if _sel_cls else _all_cls
        except Exception:
            pass
    ...
    if _cls_btn:
        ...
        try:
            ...
            st.session_state["cls_settings_stored"] = _s_cls
        ...
        if not _wc_ready:
            ...
            if not _cal_prot_ready:
                ...
                _X_p_on_cls = np.array([
                    ...
                    for i in range(_X_p_proc.shape[0])
                ])
                _C_prot_cal = np.zeros((_X_p_on_cls.shape[0], len(_cls_labels)))
                ...
                if _peg_i_wc is not None and _cal_X_peg_raw is not None and _cal_y_peg is not None:
                    ...
                    _X_peg_on_cls = np.array([
                        np.interp(_wn_cls_wc, _wn_peg_cal[_sort_peg], _X_peg_proc_c[i][_sort_peg])
                        for i in range(_X_peg_proc_c.shape[0])
                    ])
                ...
                if _R_protein_wc is None:
                    ...
                    with _wc_met_col:
                        ...
                        st.caption(
                            "Response factors calibrated by re-preprocessing your training "
                            "spectra with CLS settings and unmixing against the CLS reference set."
                        )
    ...
    if not linescan_files:
        ...
        _PIPE_PRESETS = {
            ...
            "Amide I band deconvolution": (
                ...
                {"Spectral cut #2": {"wn_min": 1600, "wn_max": 1700, "use_gap": False}},
            ),
        ...
        def _apply_pipeline(X, wn, items, pfx, fine_lo=None, fine_hi=None):
            ...
            for _item in _main:
                ...
                if _sk == "spectral_cut":
                    ...
                    _sth = st.session_state.get("spike_threshold", 8.0)
                    for _i in range(X.shape[0]):
                        X[_i] = rms.spike_removal_scp(X[_i], threshold=_sth)
        ...
        _items_amide = _pipeline_ui(
            ...
            seed_defaults={"Spectral cut #2": {"wn_min": 1600, "wn_max": 1700, "use_gap": False}})
        ...
        _use_pdb = _pc1.toggle("PDB area priors", value=False, key="use_pdb",
                               help="Use secondary structure fractions from a PDB entry to set "
                                    "physically motivated starting amplitudes (p₀) for the Gaussian "
                                    "components. Helix, sheet, and loop fractions are fetched from "
                                    "the PDBe REST API and converted to area fractions, from which "
                                    "amplitudes are derived as A₀ = f × total_area / (σ₀ × √2π). "
                                    "The optimiser then fits the spectrum freely within the FWHM "
                                    "bounds — the PDB values influence where the search starts, "
                                    "not where it ends. Without PDB priors, equal-area starting "
                                    "amplitudes are used.")
        ...
        if _run_amide:
            ...
            if _X_am.shape[1] < _n_gauss * 3:
                ...
                try:
                    # Fit mean spectrum first (warm start for individual fits)
                    with st.spinner("Fitting mean spectrum…"):
                        _popt_mean, _ = _curve_fit(
                            _multi_gauss, _wn_am, _mean_sp,
                            p0=_p0, bounds=(_blo, _bhi), maxfev=20000,
                        )
                    # Fit each individual spectrum using mean fit as p0
                    _ind_params, _ind_ok, _ind_r2 = [], [], []
                    with st.spinner(f"Fitting {_X_am.shape[0]} individual spectra…"):
                        for _si in range(_X_am.shape[0]):
                            try:
                                _po, _ = _curve_fit(
                                    _multi_gauss, _wn_am, _X_am[_si],
                                    p0=_popt_mean, bounds=(_blo, _bhi),
                                    maxfev=5000,
                                )
                                _ss_res = np.sum(
...
with tab_download:
    ...
    if not _tab_dl_has_mcr_pls and not _tab_dl_has_cls:
        ...
        if not _pub_has:
            ...
            # ── Collect row specs ─────────────────────────────────────────
            # Format: ([(w_in, h_in), ...], draw_fn)
            # Rows are stacked top-to-bottom; panels within a row are placed
            ...
            for _sv_fn, _sv_r in _res_sv.items():
                ...
                _has_pls_sv = (_sv_r.get("pls_protein") is not None
                               and _pls_p_sv is not None)
                _safe_fn    = os.path.splitext(_sv_fn)[0]

                # Row A: preprocessed spectra (+ MCR recovered spectra if MCR ran)
                def _draw_spec_row(axs, r=_sv_r, has_mcr=_has_mcr_sv):
                    ...
                    _style_ax(axs[0])
                    if has_mcr and len(axs) > 1:
                        for _k in range(r["ST_mcr"].shape[0]):
                            _lb = (_clabels_sv[_k] if _k < len(_clabels_sv)
                                   else f"Comp {_k+1}")
                ...
                if _has_mcr_sv:
                    _cap_spec += " b) MCR-recovered pure-component spectra."
                _cap_parts.append(_cap_spec)

                # Row B: MCR concentration scores vs distance
                if _has_mcr_sv:
                    def _draw_mcr_scores(axs, r=_sv_r):
                    ...
                    _cap_parts.append(
                        ...
                        f"{_nc} component(s) vs {_dlbl_sv.lower()}."
                    )

                # Row C: PLS concentration profile + co-localisation scatter
                if _has_pls_sv:
                    _has_peg_sv  = _dual_sv and _sv_r.get("pls_peg") is not None
                    _has_salt_sv = (_pls_s_sv is not None
                                    and _sv_r.get("pls_salt") is not None)
                    _cv_p_sv     = (_pls_p_sv["cv_rmse_protein"] if _dual_sv
                                    else _pls_p_sv["cv_rmse"])

                    def _draw_pls_row(
                        axs, r=_sv_r,
                        has_peg=_has_peg_sv, has_salt=_has_salt_sv,
                        cv_p=_cv_p_sv, unit=_unit_sv, dlbl=_dlbl_sv,
                    ):
                        ...
                        if has_peg:
                            ...
                            if len(axs) > 1:
                                axs[1].errorbar(
                                    _prot, _peg,
                                    xerr=cv_p, yerr=_cv_pg,
                                    fmt="o", ms=2, color=_MCOLS[2], alpha=0.6,
                                ...
                                axs[1].tick_params(axis="x", colors=_MCOLS[0],
                                                   labelsize=_TS)
                                axs[1].tick_params(axis="y", colors=_MCOLS[2],
                                                   labelsize=_TS)
                                _style_ax(axs[1])
                            ...
                            _axtw2.tick_params(axis="y", colors=_MCOLS[3],
                                               labelsize=_TS, width=0.5, length=2)
                            for _sp in _axtw2.spines.values():
                                _sp.set_linewidth(0.5)
                            if len(axs) > 1:
                                axs[1].errorbar(
                                    _prot, _salt,
                                    xerr=cv_p, yerr=_cv_s,
                                    fmt="o", ms=2, color=_MCOLS[3], alpha=0.6,
                                    ecolor="black", elinewidth=0.5,
                                    capsize=2, capthick=0.5, zorder=3,
                                )
                                axs[1].set_xlabel(f"Protein ({unit})",
            ...
            def _rmse_row(ax, pp_or_ps, label="RMSE"):
                ...
                _nc_max = len(pp_or_ps["rmse_cv_all"])
                if _nc_max > _nc_opt:
                    ax.axvspan(_nc_opt + 0.5, _nc_max + 0.5,
            ...
            if _pls_p_sv is not None:
                ...
                _nf_cal = len(_wn_vld) if _wn_vld is not None else 0
                _wn_rng = (f"{_wn_vld[0]:.0f}\u2013{_wn_vld[-1]:.0f} cm\u207b\xb9"
                           if _wn_vld is not None else "")

                # ── Training spectra ─────────────────────────────────────
                def _draw_pls_tr_spec(axs, pp=_pls_p_sv, unit=_unit_sv,
                                      dual=_dual_sv):
                    _wn = pp["wn"]
                    if dual:
                        _Xprot = pp["X_prot_proc"]
                        _Xpeg  = pp.get("X_peg_proc",
                ...
                if _dual_sv:
                    ...
                    _cap_parts.append(
                        ...
                        f"b) Molecular crowder standards ({_n_peg} spectra, green)."
                    )
                else:
                ...
                # ── Regression + RMSE ─────────────────────────────────────
                if not _dual_sv:
                    def _draw_pls1_reg_rmse(axs, pp=_pls_p_sv, unit=_unit_sv):
                        _yt = pp["y_raw"]; _yp = pp["y_pred_train"]
                    ...
                    _cap_parts.append(
                        ...
                        f"R\u00b2 = {_pls_p_sv['r2_train']:.4f}. "
                        f"b) CV RMSE (red) and training RMSE (blue); "
                        f"opt = {_pls_p_sv['n_components']}, "
                        f"CV RMSE = {_pls_p_sv['cv_rmse']:.4f} {_unit_sv}. "
                        f"Red shading: overfitting region."
                    )
                else:
                    def _draw_dual_scatters(axs, pp=_pls_p_sv, unit=_unit_sv):
                ...
                # ── Loadings (separate row) ───────────────────────────────
                def _draw_pls_loadings(axs, pp=_pls_p_sv, wn_v=_wn_vld):
                    if wn_v is not None:
                        for _i in range(pp["n_components"]):
                            axs[0].plot(wn_v, pp["model"].x_loadings_[:, _i],
                                        color=_MCOLS[_i % len(_MCOLS)],
                                        lw=0.6, label=f"LV{_i+1}")
                ...
                # ── Regression coefficients — one panel per output ────────
                if _wn_vld is not None and _nf_cal > 0:
                    _coef_all = _pls_coef(_pls_p_sv["model"], _nf_cal)
            ...
            if _pls_s_sv is not None:
                ...
                _nf_sv_s  = len(_wn_sv_sv) if _wn_sv_sv is not None else 0
                _wn_s_rng = (f"{_wn_sv_sv[0]:.0f}\u2013{_wn_sv_sv[-1]:.0f} cm\u207b\xb9"
                             if _wn_sv_sv is not None else "")

                def _draw_salt_spec(axs, ps=_pls_s_sv):
                ...
                def _draw_salt_reg_rmse(axs, ps=_pls_s_sv):
                    _ys = ps["y_raw"]; _ysp = ps["y_pred_train"]
                    axs[0].scatter(_ys, _ysp, s=10, color=_MCOLS[3], alpha=0.8,
                                   edgecolors="black", linewidths=0.3, zorder=3)
                ...
                def _draw_salt_loadings(axs, ps=_pls_s_sv, wn_v=_wn_sv_sv):
                    if wn_v is not None:
                        for _i in range(ps["n_components"]):
                            axs[0].plot(wn_v, ps["model"].x_loadings_[:, _i],
                                        color=_MCOLS[_i % len(_MCOLS)],
                                        lw=0.6, label=f"LV{_i+1}")
                        axs[0].axhline(0, color="black", lw=0.3, ls="--")
                        axs[0].set_xlabel("Wavenumber (cm\u207b\xb9)")
                ...
                if _wn_sv_sv is not None and _nf_sv_s > 0:
            ...
            if _pca_sv:
                def _draw_pca(axs, pr=_pca_sv):
                    ...
                    axs[0].scatter(
                        _sc[:, 0],
                        _sc[:, 1] if _sc.shape[1] > 1 else _sc[:, 0],
                        s=4, c=_cv_c, cmap="viridis", linewidths=0,
                    )
                    axs[0].set_xlabel(f"PC1 ({pr['var_exp'][0]:.1f}%)")
                    axs[0].set_ylabel(
                        f"PC2 ({pr['var_exp'][1]:.1f}%)"
                        if _sc.shape[1] > 1 else "PC1"
                    )
                    _style_ax(axs[0])
                    for _k in range(pr["loadings"].shape[0]):
                        axs[1].plot(pr["wn"], pr["loadings"][_k],
                                    color=_MCOLS[_k % len(_MCOLS)],
                                    lw=0.6, label=f"PC{_k+1}")
                    ...
                    for _sp in _ax3b.spines.values():
                        _sp.set_linewidth(0.5)
                    _style_ax(axs[2])
                ...
                _cap_parts.append(
                    ...
                    f"({_pca_sv['wn'][0]:.0f}\u2013{_pca_sv['wn'][-1]:.0f} cm\u207b\xb9). "
                    f"a) Score plot coloured by PC1. "
                    f"b) PC loadings ({_npc} components). "
                    f"c) Explained variance (bars) and cumulative (line); "
                    f"{_npc} PCs = {sum(_pca_sv['var_exp']):.1f}% total variance."
                )
            ...
            if _amide_sv and _amide_sv.get("popt") is not None:
                ...
                _cap_parts.append(
                    ...
                    f"({_amide_sv['wn'][0]:.0f}\u2013"
                    f"{_amide_sv['wn'][-1]:.0f} cm\u207b\xb9). "
                    f"Mean spectrum (black) with {_amide_sv['n_gauss']} "
                    f"Gaussian components (coloured fills) and total fit "
                    f"(red dashed). Labels: "
                    f"{', '.join(_amide_sv.get('g_labels', []))}."
                )
            ...
            # ── Assemble figure ───────────────────────────────────────────
            if not _row_specs:
                st.info("No figures to export yet.")
            else:
                # Compute figure size from panel sizes + spacing
                _rw_in = [
                    sum(w for w, h in ps) + max(0, len(ps) - 1) * _HPAD
                    for ps, _, _sty in _row_specs
                ]
                _rh_in = [max(h for w, h in ps) for ps, _, _sty in _row_specs]
                _fig_w_in = max(_rw_in) + _ML + _MR
                _fig_h_in = (sum(_rh_in)
                             + max(0, len(_rh_in) - 1) * _VPAD
                             + _MT + _MB)

                _fig_pub = plt.figure(figsize=(_fig_w_in, _fig_h_in))
                ...
                _cap_txt = "\n\n".join(
                    f"Figure {_fi+1}. {_c}"
                    for _fi, _c in enumerate(_cap_parts)
                )
...
with tab_tutorial:
    ...
    with st.expander("Step-by-step guide", expanded=True):
        st.markdown(
            ...
            "- **Cross-validation** — number of folds (default 5), max latent variables (default 20), and random seed (default 42).\n\n"
            "**Model metrics** — after building, five statistics are reported for each output:\n"
            "- **RMSEC** — root-mean-square error on the training (calibration) set.\n"
            "- **RMSECV** — root-mean-square error from cross-validation at the optimal component count "
            "(per output for dual/triple models — protein and crowder have separate RMSECV values).\n"
            "- **RMSEP** — root-mean-square error on the held-out 20 % test set.\n"
            "- **R²** — coefficient of determination on the training set.\n"
            "- **Q²** — R² on the held-out 20 % test set; measures true predictive power "
            "(Massei et al., *J. Pharm. Biomed. Anal.* 2026).\n\n"
            "**Mass balance (estimated water content)** — below the PLS linescan plots, "
            "protein (mg/mL) and crowder concentrations are converted to mass fractions and "
            "the water fraction is estimated as 1 − f_protein − f_crowder. "
            "This requires a solution density (g/mL). "
            "A **Protein MW (kDa) — density estimate** helper is provided: enter the protein "
            "molecular weight in kDa and the estimated intrinsic protein density from "
            "Fischer et al. (2004), ρ = 1.410 + 0.145·exp(−MW/13) g/cm³, is shown as a live "
            "reference — use it to guide the density input.\n\n"
            ...
            "4. Set the **Protein solution density (g/mL)** used to convert mg/mL to mass fraction. "
            "A **Protein MW (kDa) — density estimate** helper is available: enter the protein MW "
            "in kDa and the live caption shows ρ = 1.410 + 0.145·exp(−MW/13) g/cm³ "
            "(Fischer et al., 2004) as a reference value.\n"
            "5. Mass fraction profiles for all linescan files are computed and plotted automatically.\n"
    ...
    with st.expander("How the analysis works — methods"):
        ...
        st.markdown(
            ...
            "**Reported metrics** — five statistics are shown after building any PLS model:\n\n"
            "- **RMSEC** and **R²** — calibration performance on the training set.\n"
            "- **RMSECV** — cross-validation error at the optimal number of LVs. "
            "For dual/triple models, RMSECV is computed per output using only the non-zero rows "
            "in the stacked matrix, so protein and crowder get independent values.\n"
            "- **RMSEP** — prediction error on the held-out 20 % test set.\n"
            "- **Q²** — R² on the held-out 20 % test set; defined as "
            "1 − Σ(y_test − ŷ_test)² / Σ(y_test − ȳ_test)² "
            "(Massei et al., *J. Pharm. Biomed. Anal.* 2026). "
            "Q² measures true predictive power, not cross-validation fit.\n\n"
        ...
        st.markdown(
            "##### Classical Least Squares (CLS) unmixing\n"
            "CLS solves a **non-negative least squares (NNLS)** problem per spectrum:\n\n"
            "&nbsp;&nbsp;&nbsp;&nbsp;**x ≈ c · Sᵀ**,&nbsp;&nbsp; c ≥ 0\n\n"
            "where **x** is a single preprocessed spectrum, **Sᵀ** is the matrix of "
            "preprocessed pure-component reference spectra, and **c** is the vector of "
            "non-negative concentrations (contributions) to be determined. "
            "This is solved independently for every spectrum in the linescan.\n\n"
            "Unlike MCR-ALS, CLS has **no iterative fitting**: the solution is unique given "
            "the reference set and the non-negativity constraint. There are no convergence "
            "parameters to tune. The quality of the result depends entirely on how well the "
            "reference spectra span the true spectral variance of the sample.\n\n"
            "**Fit statistics reported per spectrum:**\n"
            "- **R²** — fraction of spectral variance explained by the CLS reconstruction.\n"
            "- **RMSE** — root-mean-square error between measured and reconstructed spectrum.\n"
            "- **Residual** — the unexplained spectral signal (measured − reconstructed). "
            "Non-zero residual indicates spectral variance not captured by the reference set — "
            "inspect this to diagnose missing components or reference mismatch.\n\n"
            "**Important:** the concentrations are in arbitrary units (not fractions or physical "
    ...
    with st.expander("File formats"):
        st.markdown(
            ...
            "**CLS reference CSV** *(CLS unmixing tab)*  \n"
            "Same format as the MCR reference CSV: first column = component label, "
            "remaining columns = reference spectrum intensities. "
            "The spectra are preprocessed with the CLS normalisation setting before NNLS. "
            "Choose the normalisation in the Files tab that matches how your reference spectra "
            "were originally acquired and processed — Area (unit area) is recommended.\n\n"
...
- [Dieing & Hollricher, *Spectroscopy* (2008)](https://www.spectroscopyonline.com/view/basic-aspects-experimental-design-raman-microscopy) — experimental design guidelines for Raman microscopy
    ...
    st.markdown(
        ...
        "|---|---|---|\n"
        "| [scikit-learn](https://scikit-learn.org) | PLS regression, cross-validation, component selection | [Pedregosa et al., 2011](https://jmlr.org/papers/v12/pedregosa11a.html) |\n"
        "| [pyMCR](https://pages.nist.gov/pyMCR/) | MCR-ALS spectral decomposition | [Camp, 2019](https://doi.org/10.6028/jres.124.018) |\n"
        "| [RamanSPy](https://ramanspy.readthedocs.io) | Baseline correction (ARPLS, AIRPLS, IASLS, DRPLS, polynomial variants) | [Georgiev et al., 2024](https://doi.org/10.1021/acs.analchem.4c00383) |\n"
        "| [chemotools](https://chemotools.org) | ALS baseline, SNV / min-max / vector normalisation, Savitzky-Golay smoothing and second derivative | [Cabaneros Lopez, 2024](https://doi.org/10.21105/joss.06802) |\n"
        "| [SciPy](https://scipy.org) | Rolling-ball baseline, Gaussian smoothing, FFT low-pass filter, peak fitting, NNLS (CLS unmixing) | [Virtanen et al., 2020](https://doi.org/10.1038/s41592-019-0686-2) |\n"
        "| Custom | Spike removal (Whitaker-Hayes modified Z-score) | [Whitaker & Hayes, 2018](https://doi.org/10.1016/j.chemolab.2018.06.009) |\n"
        "| Custom | Rubberband (convex hull) baseline, endpoint / linear baseline, area normalisation | — |\n"
        "| Custom | Protein density estimator: ρ(M) = 1.410 + 0.145·exp(−M/13) g/cm³ | [Fischer, Polikarpov & Craievich, *Protein Sci.* 2004](https://doi.org/10.1110/ps.04688204) |\n"
        "| [NumPy](https://numpy.org) | Numerical array operations | [Harris et al., 2020](https://doi.org/10.1038/s41586-020-2649-2) |\n"
        "| [pandas](https://pandas.pydata.org) | Data handling and Excel export | [McKinney, 2010](https://doi.org/10.25080/Majora-92bf1922-00a) |\n"
        "| [Plotly](https://plotly.com/python/) | Interactive figures | Plotly Technologies Inc., 2015 |\n"
        "| [Streamlit](https://streamlit.io) | Web application framework | — |\n"
        "| [streamlit-sortables](https://github.com/ohtaman/streamlit-sortables) | Drag-and-drop pipeline reordering | — |\n"
    )
